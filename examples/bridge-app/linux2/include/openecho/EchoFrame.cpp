...
EchoFrame::EchoFrame(const std::string srcEchoAddress,
	...
	mTID += (short unsigned int)(frame.at(3) & 0xFF);
	mSrcEchoClassCode = (frame.at(4) << 8) & 0xFF00;
	mSrcEchoClassCode += (short unsigned int)(frame.at(5) & 0xFF);
	mSrcEchoInstanceCode = frame.at(6);
	mDstEchoClassCode = (frame.at(7) << 8) & 0xFF00;
	mDstEchoClassCode += (short unsigned int)(frame.at(8) & 0xFF);
...
std::vector<unsigned char> EchoFrame::getFrameByteArray() const {
	...
	int propertyListSize = (int)mPropertyList.size();
	...
	frame.push_back((unsigned char)(((mTID >> 8) & 0xff)));
	frame.push_back((unsigned char)((mTID & 0xff)));
	frame.push_back((unsigned char)(((mSrcEchoClassCode >> 8) & 0xff)));
	frame.push_back((unsigned char)((mSrcEchoClassCode & 0xff)));
	frame.push_back(mSrcEchoInstanceCode);
	frame.push_back((unsigned char)(((mDstEchoClassCode >> 8) & 0xff)));
	frame.push_back((unsigned char)((mDstEchoClassCode & 0xff)));
	frame.push_back(mDstEchoInstanceCode);
	frame.push_back(mESV);
	frame.push_back((unsigned char)(propertyListSize));
	for(int pidx = 0; pidx < (int)mPropertyList.size(); pidx++) {
